...
def hydra_main(cfg: TrainingUnitEncoderConfig):
    ...
    device = torch.device("cuda" if torch.cuda.is_available()
                          and not cfg.train.train_no_cuda else "cpu")
    if device.type == "cpu" and not cfg.train.train_no_cuda:
        ...
        with tqdm(train_loader, total=len(train_dataset)//cfg.train.batch_size) as progress_bar:
            for batch_idx, batch in enumerate(progress_bar):
                ...
                if cfg.train.fp16_run:
                    ...
                    text_encoder_grad_norm = torch.nn.utils.clip_grad_norm_(text_encoder.parameters(), max_norm=1)
                    duration_predictor_grad_norm = torch.nn.utils.clip_grad_norm_(duration_predictor.parameters(), max_norm=1)
                ...
                writer.add_scalar("Train1/duration_loss", dur_loss.item(), global_step=iteration)
                ...
                writer.add_scalar("Train1/duration_predictor_grad_norm", duration_predictor_grad_norm, global_step=iteration)
...
def compute_train_step_loss(cfg: TrainingUnitEncoderConfig,
    ...
    spk_emb_sample  = batch['spk_emb'].cuda()                                 # SPEAKER EMBEDDING
    ...
    with torch.no_grad():
        ...
        y_mu_double = torch.matmul(
            2.0 * (factor * mu_x).transpose(1, 2), y_sample)
    ...
    if not isinstance(out_size, type(None)) and out_size < y_max_length:
        ...
        y_cut = torch.zeros(
            y_sample.shape[0], cfg.data.n_feats, out_size, dtype=y_sample.dtype, device=y_sample.device)
        y_cut_lengths = []
        for i, (y_, out_offset_) in enumerate(zip(y_sample, out_offset)):
            y_cut_length = out_size + \
                (y_sample_lengths[i] - out_size).clamp(None, 0)
